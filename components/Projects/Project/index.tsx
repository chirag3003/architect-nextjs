...
import demoProject from "../../../configs/projects/demo";

function Project() {
    return (
        <>
            <div className={"image-purple w-full max-h-screen"}>
                <img src={demoProject.image} className="image lg w-full max-h-screen object-cover"
                     alt={""}/>
                <article
                    className="content absolute top-0 left-0 w-full h-full hidden lg:flex flex-col justify-center items-center text-white">
                    <Title1 title={demoProject.title} heading={demoProject.subtitle}/>
                    {demoProject.dParas.map((text, index) => {
            ...
            <section id="images" className={" p-10 lg:px-20"}>
                <Images images={demoProject.images}/>
            </section>
        </>
    );
}
