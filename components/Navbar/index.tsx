    ...
    >
      <div className="nav-container flex items-center justify-between relative w-full h-full py-6 px-4 lg:px-12">
            ...
            {socialLinks.map((item, index) => {
              return (
                <Link key={index} className={"social-link"} href={item.link}>
                  ...
                  {item.title && (
                    <span className={"sr-only"} itemProp={item.title}>
                      {item.title}
                    </span>
                  )}
                </Link>
        ...
        <button
          ...
          className={
            "outline-none border-none focus:outline-none focus:border-none lg:hidden"
          }
        >
          <Bars3Icon className={"font-bold text-lg"} height={20} />
        </button>
      </div>
      <div
        id={"sidebar"}
        style={{ transform: `translateX(${sidebar ? "0" : "100%"})` }}
        className="fixed lg:hidden top-0 right-0 bg-white h-screen w-2/3 max-w-sm z-40 text-black shadow transition-all"
      >
        <div className="header py-8 px-4 flex items-center justify-end shadow-sm">
          ...
          >
            <XMarkIcon className={"font-bold text-lg"} height={20} />
          </button>
        </div>
          ...
          {socialLinks.map((item, index) => {
            return (
              <Link key={index} className={"social-link"} href={item.link}>
                ...
                {item.title && (
                  <span itemProp={item.title} className={"sr-only"}>
                    {item.title}
                  </span>
                )}
              </Link>
